local opts = {
	...
	current_line_blame_formatter = function(name, blame_info)
		...
		if days_diff < year_in_days then
			...
			elseif
				current_day.year == author_day.year and current_day.yday - 1 == author_day.yday or days_diff == 1
			then
				author_string = string.format(
					"%s, Yesterday at %s - %s",
					author,
			...
			elseif days_diff < 30 then
				author_string = string.format("%s, %d days ago - %s", author, days_diff, blame_info.summary)
			-- If more than a month show months
			else
				author_string =
					string.format("%s, %d months ago - %s", author, math.floor(days_diff / 30), blame_info.summary)
			end
		-- If more than a year show actual date
		else
			author_string =
				string.format("%s, %s - %s", author, os.date("%d %b %Y", blame_info.author_time), blame_info.summary)
		end
